	...
	nmcln.on('ready', function(){
	    ...
	    appHttp.use('/vadmin/2', function(req, res, next){	        
            ...
            if (req.query && req.query.url){
                if (self.addURL(req.query.url, {purl: req.query.purl})){
                    res.writeHead(301, {'location': self.proxyURL[req.query.url]});
		            res.end();
                } else {
                    next('invalid param');
                }
	    ...
	    self.proxyCache[vurle].on('proxyResponse', function(req, res, response){
						...
						*/
                        ...
                        resstr = resstr.replace(/(<head(\s|\S)*\/head>)|(<body(\s|\S)*\/body>)/gi, function(hb){   
	                        ...
	                        hb = hb.replace(REGEX_URL, function(href){
                                ...
                                var hrefvurl = self.addURL(hrefhost, {purl: self.proxyURL[prxself.urle]});
                        ...
                        if (!resstr.match(/(<head(\s|\S)*\/head>)|(<body(\s|\S)*\/body>)/gi)) {   
                            if (debug) console.log('other-body:'+hb);    
		    ...
		    if ((response.statusCode >= 300 && response.statusCode < 400) &&
		        ...
		        if (!(sameHost && samePort && sameProto)) {
		            ...
		            var locvurl = self.addURL(response.headers.location, {purl: self.proxyURL[prxself.urle]});
