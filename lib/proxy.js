	...
	nmcln.on('ready', function(){
	    ...
	    appHttp.use('/vadmin/3', function(req, res, next){	        
            ...
            if (req.query && req.query.url){
                ...
                for (var idx in urls) {
                    if (self.addURL(urls[idx], {purl: req.query.purl})) {
                        vobj[urls[idx]] = self.proxyURL[urls[idx]];    
                    }
                }
...
Proxy.prototype.addURL = function(urle, options){
    var self  = this;
    var urls  = URL.parse(urle.match(/^(http:\/\/)|^(https:\/\/)/gi)? urle : 'http://'+urle, true, true);
    ...
    // check urls
    if (!host) {
        console.log('invalid URL to proxy: '+urle);
        return;
    }
						...
						*/
                        ...
                        resstr = resstr.replace(/(<head(\s|\S)*\/head>)|(<body(\s|\S)*\/body>)/gi, function(hb){   
	                        ...
	                        if ((/<head(\s|\S)*\/head>/gi).test(hb)) {
			                    ...
			                    hb = hb.replace(/<\/head>/gi, function(html){
		                            var text = FS.readFileSync(__dirname+'/public/peer-proxy.js').toString();
		                            
									if (debug) console.log('head:'+text+html);
