...
var Proxy = module.exports = function(websites, fn, options){ 
	...
	nmcln.on('ready', function(){
	    ...
	    // 4.3
	    // portal page
	    // notes: to enable debug mode append /vdebug[/:mode] in req.url
	    appHttp.use(function(req, res, next){
            ...
            for (var site in self.proxyURL) {
                ...
                if (req.url.match('/vdebug') && self.proxyPrn[site]) continue;
...
Proxy.prototype.addURL = function(urle, options){
    ...
    if (!self.proxyCache[vurle]) {
	    ...
	    self.proxyCache[vurle].on('proxyResponse', function(req, res, response){
	        ...
	        // 3.
	        // rewrite href from text/html or application/json response for whole website proxy
                    ...
                    _decomp.on('end', function() {		
						...
						*/
                        ...
                        if (response.headers['content-type'].match('text/html') ||
                            ...
                            // rewrite href host part of application/json
                            resstr = resstr.replace(REGEX_URL, function(href){
                                ...
                                // add proxy for child href 
                                var hrefvurl = self.addURL(hrefhost, {purl: prxself.urle});
                                if (debug) console.log('zipped application/json:'+hrefhost+' -> '+hrefvurl);
