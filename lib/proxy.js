...
var Proxy = module.exports = function(websites, fn, options){ 
	...
	nmcln.on('ready', function(){
	    ...
	    // 4.2.2
	    // admin service, checking admin node like 1XX,2XX,3XX,4XX,5XXX,etc
	    // notes: to enable admin mode append /vadmin[/:mode] in req.url
	    appHttp.use(function(req, res, next){
	        // - 1XX: add/remove website to/from proxy, like /vadmin/1/?action=add/remove&url=xxx&purl=xxx 
	        if (req.url.match('/vadmin/1')) {
	            if (debug) console.log('req.query:'+JSON.stringify(req.query));
	            
	            if (req.query && req.query.url &&
	                (req.query.action == 'add' || req.query.action == null)){
	                self.addURL(req.query.url, {purl: req.query.purl});
	            }
	            
	            if (req.query && req.query.url &&
	                (req.query.action == 'remove' || req.query.action == 'delete')){
	                self.delURL(req.query.url);
	            }
	        } else if (req.url.match('/vadmin')) {
	            var htmlstr = '';
	            
	            htmlstr += '<ol>';
	            for (var site in self.proxyURL) {
	                if (debug) console.log('plink of '+site+': '+self.proxyPrn[site]);
	                if (req.url.match('/vadmin') && self.proxyPrn[site]) continue;
...
Proxy.prototype.addURL = function(urle, options){
    ...
    // check urls
    if (urls == null) {
        console.log('invalid URL to proxy');
        return;
    }
    ...
    if (!self.proxyCache[vurle]) {
	    ...
	    // hook purl,urle,vurle,purl in httpProxy instance
	    self.proxyCache[vurle].purl  = purl;
	    ...
	    self.proxyCache[vurle].on('proxyResponse', function(req, res, response){
						...
						*/
                        ...
                        resstr = resstr.replace(/(<head(\s|\S)*\/head>)|(<body(\s|\S)*\/body>)/gi, function(hb){   
	                        ...
	                        hb = hb.replace(REGEX_URL, function(href){
                                ...
                                var hrefvurl = self.addURL(hrefhost, {purl: prxself.urle});
	                        ...
	                        // embedded peer-proxy.js in body and jquery.js in head
	                        if ((/<body(\s|\S)*\/body>/gi).test(hb)) {
		                        hb = hb.replace(/<\/body>/gi, function(html){
		                            var js = '';
		                            return js + html;
		                        });
	                        }
		    ...
		    if ((response.statusCode >= 300 && response.statusCode < 400) &&
		        ...
		        if (!(sameHost && samePort && sameProto)) {
		            ...
		            var locvurl = self.addURL(response.headers.location, {purl: prxself.urle});
