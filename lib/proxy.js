...
// -  options: user custom parameters, like {keyword: ..., usrkey: ..., domain: ..., endpoints: ..., turn: ...}
...
var Proxy = module.exports = function(websites, fn, options){ 
	...
	nmcln.on('ready', function(){
	    ...
	    // cache, ...
	    appHttp.use(Connect.staticCache({maxLength: 512*1024, maxObjects: 128}));
...
Proxy.prototype.addURL = function(urle, options){
    ...
    if (!(self.proxyCache[vurle] && self.proxyURL[urle])) {
	    ...
	    self.proxyCache[vurle].on('proxyResponse', function(req, res, response){
	        ...
	        if ((response.statusCode === 200) && 
	            ...
	            if (('content-encoding' in response.headers) &&
                    ...
                    _decomp.on('end', function() {                
                        if (debug > 1) console.log('text response:'+resbuf);
		                ...
		                if (debug > 1) console.log('overwrote text response:'+resbuf);
            ...
            var regexsc1 = new RegExp('domain=[^;]+;', 'gi');
            var regexsc2 = new RegExp('domain=[^;]+', 'gi');
